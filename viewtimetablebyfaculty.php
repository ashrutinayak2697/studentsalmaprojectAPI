...
$input_params = json_decode($jsonInput,true);
  ...
  {
    $a=1;
    $DocumentQuery=mysql_query("SELECT * FROM timetable inner join reg_faculty on reg_faculty.fac_id=timetable.faculty_id inner join subject on subject.subject_id=timetable.subject_id where  timetable.faculty_id='".$faculty_id."' and timetable.day='".$day."'");
